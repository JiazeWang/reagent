...
from registration.model_pn_2D_2048 import Agent
import registration.model_pn_2D_2048 as util_model
...
def train(agent, logger, dataset, noise_type, epochs, lr, lr_step, alpha, model_path, reward_mode=""):
    ...
    for epoch in range(epochs):
        ...
        if chamfer_test <= best_chamfer:
            ...
            util_model.save(agent, f"{model_path}_2048.zip", infos)
...
if __name__ == '__main__':
    ...
    logger = Logger(log_dir=os.path.join(code_path, f"logs/{dataset}/"), log_name=f"2048_{mode}",
    ...
    if args.mode == "pretrain" and dataset == "m40":
        ...
        if dataset == "m40":
            ...
            if os.path.exists(os.path.join(code_path, f"weights/m40_pretrain_2048.zip")):
                util_model.load(agent, os.path.join(code_path, f"weights/m40_pretrain_2048.zip"))
            else:
