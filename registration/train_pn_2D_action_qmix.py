...
from registration.model_pn_2D_action_qmixv1 import Agent
import registration.model_pn_2D_action_qmixv1 as util_model
...
def train(agent, logger, dataset, noise_type, epochs, lr, lr_step, alpha, model_path, reward_mode=""):
    ...
    for epoch in range(epochs):
        ...
        if chamfer_test <= best_chamfer:
            ...
            util_model.save(agent, f"{model_path}_qmixv1.zip", infos)
...
if __name__ == '__main__':
    ...
    logger = Logger(log_dir=os.path.join(code_path, f"logs/{dataset}/"), log_name=f"qmixv1_{mode}",
    ...
    if args.mode == "pretrain" and dataset == "m40":
        ...
        if dataset == "m40":
            ...
            if os.path.exists(os.path.join(code_path, f"weights/m40_pretrain_qmixv1.zip")):
                util_model.load(agent, os.path.join(code_path, f"weights/m40_pretrain_qmixv1.zip"))
            else:
