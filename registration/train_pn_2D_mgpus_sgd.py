...
def train(agent, logger, dataset, noise_type, epochs, lr, lr_step, alpha, model_path, reward_mode=""):
    ...
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, lr_step, 0.1)
    ...
    for epoch in range(epochs):
        ...
        if chamfer_test <= best_chamfer:
            ...
            util_model.save(agent, f"{model_path}_mgpu_shared_sgd.zip", infos)
...
if __name__ == '__main__':
    ...
    logger = Logger(log_dir=os.path.join(code_path, f"logs/{dataset}/"), log_name=f"sgd_shared_mGPUs_2D_pn_{mode}",
    ...
    if args.mode == "pretrain" and dataset == "m40":
        ...
        train(agent, logger, dataset, noise_type="clean", epochs=100, lr=1e-2, lr_step=20, alpha=0,
        ...
        if dataset == "m40":
            ...
            if os.path.exists(os.path.join(code_path, f"weights/m40_pretrain_mgpu_shared_sgd.zip")):
                util_model.load(agent, os.path.join(code_path, f"weights/m40_pretrain_mgpu_shared_sgd.zip"))
            else:
        ...
        lr = 1e-3 if dataset == "m40" else 1e-4
