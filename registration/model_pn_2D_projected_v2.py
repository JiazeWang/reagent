...
class StateEmbed(nn.Module):
    ...
    def forward(self, src, tgt):
        ...
        emb_src_mv =  emb_src_mv.view(-1, 1024, 1).repeat(1, 1, 1024)
        emb_src = torch.cat((emb_src_p, emb_src_mv), dim=1)
        emb_src = self.projected_layer(emb_src).squeeze()
        ...
        if BENCHMARK and len(tgt.shape) != 3:
            ...
            emb_tgt_mv =  emb_tgt_mv.view(-1, 1024, 1).repeat(1, 1, 1024)
            emb_tgt = torch.cat((emb_tgt_p, emb_tgt_mv), dim=1)
            emb_tgt = self.projected_layer(emb_tgt).squeeze()
