...
def train(agent, logger, dataset, noise_type, epochs, lr, lr_step, alpha, model_path, reward_mode=""):
    ...
    for epoch in range(epochs):
        ...
        if chamfer_test <= best_chamfer:
            ...
            util_model.save(agent, f"{model_path}_projected_sgd.zip", infos)
...
if __name__ == '__main__':
    ...
    logger = Logger(log_dir=os.path.join(code_path, f"logs/{dataset}/"), log_name=f"projected_sgd_{mode}",
    ...
    if args.mode == "pretrain" and dataset == "m40":
        ...
        train(agent, logger, dataset, noise_type="clean", epochs=50, lr=1e-2, lr_step=10, alpha=0,
        ...
        lr = 1e-3 if dataset == "m40" else 1e-3
        lr_step = 15 if dataset == "m40" else 40
