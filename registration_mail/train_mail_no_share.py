...
from registration_mail.model_fusion_no_share import Agent
import registration_mail.model_fusion_no_share as util_model
...
def train(agent, logger, dataset, noise_type, epochs, lr, lr_step, alpha, model_path, reward_mode=""):
    ...
    for epoch in range(epochs):
        ...
        if chamfer_test <= best_chamfer:
            ...
            util_model.save(agent, f"{model_path}_fusion_noshare.zip", infos)
...
if __name__ == '__main__':
    ...
    logger = Logger(log_dir=os.path.join(code_path, f"logs/{dataset}/"), log_name=f"fusion_no_share_{mode}",
    ...
    if args.mode == "pretrain" and dataset == "m40":
        ...
        if dataset == "m40":
            ...
            if os.path.exists(os.path.join(code_path, f"weights/m40_pretrain_fusion_noshare.zip")):
                util_model.load(agent, os.path.join(code_path, f"weights/m40_pretrain_fusion_noshare.zip"))
            else:
