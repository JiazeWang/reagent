...
from registration_dense.model_2D import Agent
import registration_dense.model_2D as util_model
...
def train(agent, logger, dataset, noise_type, epochs, lr, lr_step, alpha, model_path, reward_mode=""):
    ...
    for epoch in range(epochs):
        ...
        if chamfer_test <= best_chamfer:
            ...
            util_model.save(agent, f"{model_path}_2D.zip", infos)
...
if __name__ == '__main__':
    ...
    logger = Logger(log_dir=os.path.join(code_path, f"logs/{dataset}/"), log_name=f"2d_{mode}",
    ...
    if args.mode == "pretrain" and dataset == "m40":
        ...
        if dataset == "m40":
            ...
            if os.path.exists(os.path.join(code_path, f"weights/m40_pretrain_2D.zip")):
                util_model.load(agent, os.path.join(code_path, f"weights/m40_pretrain_2D.zip"))
            else:
