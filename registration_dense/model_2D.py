...
class StateEmbed(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv0 = nn.Linear(1536, 1024)
    def forward(self, src, tgt):
        B, N, D = src.shape
        ...
        emb_src_mv = torch.max(emb_src_mv, 2, keepdim=True)[0].squeeze()
        #print("emb_src_mv.shape: ", emb_src_mv.shape)
        emb_src = self.conv0(emb_src_mv)
        ...
        if BENCHMARK and len(tgt.shape) != 3:
            ...
            emb_tgt_mv = torch.max(emb_tgt_mv, 2, keepdim=True)[0].squeeze()
            emb_tgt = self.conv0(emb_tgt_mv)
